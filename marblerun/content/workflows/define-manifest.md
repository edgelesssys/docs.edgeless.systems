...
In contrast, `pkg1` is identified through the triplet `SignerID`, `ProductID`, and `SecurityVersion`. `SignerID` cryptographically identifies the vendor of the package; `ProductID` is an arbitrary product ID chosen by the vendor, and `SecurityVersion` is the security-patch level of the product. See our [adding a service hands-on](content/workflows/add-service.md#step-21-define-the-enclave-software package) on how to get these values for a given service.
...
* `Shared` (default: `false`): specifies if the secret should be shared across all Marbles (`true`), or if the secret should be uniquely generated for each Marble (`false`). See [Secrets management](content/features/secrets-management.md) for more info.
...
* `UserDefined` (default: `false`): specifies if the secret should be generated by MarbleRun  (`false`), or if it will be [uploaded by a user](content/workflows/managing-secrets.md) at a later date  (`true`).
...
Users with the appropriate roles can [update a manifest](content/workflows/update-manifest.md">}}) and [read or write secrets](content/workflows/managing-secrets.md).
...
The optional entry `RecoveryKeys` holds PEM-encoded RSA public keys which can be used to recover a failed MarbleRun deployment. (The process of recovering a MarbleRun instance is described in our [recovery chapter](content/features/recovery.md)). So far, only one public key entry is supported in the current release of MarbleRun.
