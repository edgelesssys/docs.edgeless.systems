...
## Requirements
In order to deploy an update, you need to be in possession of a certificate/private key pair belonging to a user from the `Users` section of the original manifest, as described in ["defining a manifest"](content/workflows/define-manifest.md#manifestmarbles).
Furthermore the user needs to be [permitted to update](content/workflows/define-manifest.md#manifestroles) the chosen packages.
